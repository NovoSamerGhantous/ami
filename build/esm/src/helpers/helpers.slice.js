...
import { Vector3, Color, Matrix4, DoubleSide, Mesh, Vector4 } from 'three';
/**
 * @module helpers/slice
 */
class helpersSlice extends helpersMaterialMixin {
    constructor(stack, index = 0, position = new Vector3(0, 0, 0), direction = new Vector3(0, 0, 1), aabbSpace = 'IJK') {
        //
        super();
        // private vars
        this._stack = stack;
    ...
    _init() {
        ...
        if (this._aaBBspace === 'IJK') {
            ...
            this._center = new Vector3(this._stack.halfDimensionsIJK.x - 0.5, this._stack.halfDimensionsIJK.y - 0.5, this._stack.halfDimensionsIJK.z - 0.5);
    ...
    cartesianEquation() {
        ...
        let p1 = new Vector3(vertices[0].x, vertices[0].y, vertices[0].z).applyMatrix4(dataToWorld);
        let p2 = new Vector3(vertices[1].x, vertices[1].y, vertices[1].z).applyMatrix4(dataToWorld);
        let p3 = new Vector3(vertices[2].x, vertices[2].y, vertices[2].z).applyMatrix4(dataToWorld);
        let v1 = new Vector3();
        let v2 = new Vector3();
        let normal = v1
            .subVectors(p3, p2)
