 ...
 */
class helpersSlice extends helpers_material_mixin_1.helpersMaterialMixin {
    constructor(stack, index = 0, position = new three_1.Vector3(0, 0, 0), direction = new three_1.Vector3(0, 0, 1), aabbSpace = 'IJK') {
        //
        super();
        // private vars
        this._stack = stack;
    ...
    set borderColor(borderColor) {
        ...
        this._uniforms.uBorderColor.value = new three_1.Color(borderColor);
    }
    get borderColor() {
    ...
    _init() {
        ...
        if (this._aaBBspace === 'IJK') {
            ...
            this._center = new three_1.Vector3(this._stack.halfDimensionsIJK.x - 0.5, this._stack.halfDimensionsIJK.y - 0.5, this._stack.halfDimensionsIJK.z - 0.5);
            this._toAABB = new three_1.Matrix4();
        }
        else {
            // LPS
    ...
    _create() {
        ...
        // Convenience vars
        try {
            const SliceGeometryContructor = (0, geometries_slice_1.geometriesSlice)();
    ...
    cartesianEquation() {
        ...
        if (!this._geometry || !this._geometry.vertices || this._geometry.vertices.length < 3) {
            return new three_1.Vector4();
        }
        let vertices = this._geometry.vertices;
        let dataToWorld = this._stack.ijk2LPS;
        let p1 = new three_1.Vector3(vertices[0].x, vertices[0].y, vertices[0].z).applyMatrix4(dataToWorld);
        let p2 = new three_1.Vector3(vertices[1].x, vertices[1].y, vertices[1].z).applyMatrix4(dataToWorld);
        let p3 = new three_1.Vector3(vertices[2].x, vertices[2].y, vertices[2].z).applyMatrix4(dataToWorld);
        ...
        let normal = v1
            ...
            .normalize();
        return new three_1.Vector4(normal.x, normal.y, normal.z, -normal.dot(p1));
    }
}
