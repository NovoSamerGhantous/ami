...
class widgetsBase extends three_1.Object3D {
    constructor(targetMesh, controls, params) {
        ...
        this._worldPosition = new three_1.Vector3(); // LPS position
    ...
    getPointInRegion(region, point) {
        ...
        return new three_1.Vector2((point.x - region.x0 - (region.axisX || 0)) * region.deltaX, (point.y - region.y0 - (region.axisY || 0)) * region.deltaY);
    }
    /**
    ...
    getLineData(pointA, pointB) {
        ...
        const angle = line.angleTo(new three_1.Vector3(1, 0, 0));
        return {
            line,
            length,
    ...
    getRectData(pointA, pointB) {
        const line = pointB.clone().sub(pointA);
        const vertical = line.clone().projectOnVector(new three_1.Vector3(0, 1, 0));
        const min = pointA.clone().min(pointB); // coordinates of the top left corner
        return {
            width: line
                .clone()
                .projectOnVector(new three_1.Vector3(1, 0, 0))
                .length(),
