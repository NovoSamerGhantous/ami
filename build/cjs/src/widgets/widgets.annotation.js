 ...
 */
class widgetsAnnotation extends widgets_base_1.widgetsBase {
    constructor(targetMesh, controls, params = {}) {
        ...
        // var
        this._labelOffset = new three_1.Vector3(); // difference between label center and second handle
        this._mouseLabelOffset = new three_1.Vector3(); // difference between mouse coordinates and label center
        // add handles
        this._handles = [];
        let handle;
        const WidgetsHandle = (0, widgets_handle_1.widgetsHandle)();
        for (let i = 0; i < 2; i++) {
    ...
    onStart(evt) {
        if (this._labelhovered) {
            ...
            this._mouseLabelOffset = new three_1.Vector3(offsets.screenX - paddingPoint.x, offsets.screenY - paddingPoint.y, 0);
    ...
    onMove(evt) {
        if (this._movinglabel) {
            const offsets = this.getMouseOffsets(evt, this._container);
            this._labelOffset = new three_1.Vector3(this._handles[1].screenPosition.x - offsets.screenX + this._mouseLabelOffset.x, this._handles[1].screenPosition.y - offsets.screenY + this._mouseLabelOffset.y, 0);
    ...
    createMesh() {
        // material
        this._material = new three_1.LineBasicMaterial();
        this.updateMeshColor();
        // line geometry
        // this._geometry = new three.Geometry();
        // this._geometry.vertices.push(this._handles[0].worldPosition);
        // this._geometry.vertices.push(this._handles[1].worldPosition);
        this._geometry = new three_1.BufferGeometry();
        const positions = new Float32Array(2 * 3);
        this._geometry.setAttribute('position', new three_1.BufferAttribute(positions, 3));
        let index = 0;
        ...
        // line mesh
        this._meshline = new three_1.Line(this._geometry, this._material);
        this._meshline.visible = true;
        this.add(this._meshline);
        // cone geometry
        this._conegeometry = new three_1.CylinderGeometry(0, 2, 10);
        this._conegeometry.translate(0, -5, 0);
        this._conegeometry.rotateX(-Math.PI / 2);
        // cone mesh
        this._cone = new three_1.Mesh(this._conegeometry, this._material);
