 ...
 */
class widgetsFreehand extends widgets_base_1.widgetsBase {
    ...
    createMaterial() {
        this._material = new three_1.MeshBasicMaterial({ side: three_1.DoubleSide });
    ...
    updateMesh() {
        ...
        let referenceDirection = new three_1.Vector3().subVectors(points[0], center).normalize();
        let direction = new three_1.Vector3().crossVectors(new three_1.Vector3().subVectors(points[0], center), // side 1
        new three_1.Vector3().subVectors(points[1], center) // side 2
        );
        let base = new three_1.Vector3().crossVectors(referenceDirection, direction).normalize();
        let orderedpoints = [];
        // other lines // if inter, return location + angle
        for (let j = 0; j < points.length; j++) {
            let point = new three_1.Vector3(points[j].x, points[j].y, points[j].z);
            point.direction = new three_1.Vector3().subVectors(points[j], center).normalize();
        ...
        this._geometry = new three_1.ShapeGeometry(shape);
