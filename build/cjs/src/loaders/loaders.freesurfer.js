...
three_1.FreeSurferLoader = function () { };
Object.assign(three_1.FreeSurferLoader.prototype, three_1.EventDispatcher.prototype, {
    constructor: three_1.FreeSurferLoader,
    ...
    parse: function (data) {
        ...
        let faceCount = 0;
        switch (surfType) {
            case three_1.FreeSurferLoader.QUAD_FILE_MAGIC_NUMBER: {
                ...
                for (let v = 0; v < vertCount; v++) {
                    positions[index++] = reader.getInt16(offset + 0, littleEndian) / 100;
                    positions[index++] = reader.getInt16(offset + 2, littleEndian) / 100;
                    positions[index++] = reader.getInt16(offset + 4, littleEndian) / 100;
                    offset += 6;
                }
                for (let f = 0; f < faceCount; f++) {
                    ...
                    offset += 12;
                    geometry.faces.push(new three_1.Face3(v1, v2, v3));
                    geometry.faces.push(new three_1.Face3(v1, v3, v4));
                }
                geometry.computeBoundingBox();
                break;
            }
            case three_1.FreeSurferLoader.TRIANGLE_FILE_MAGIC_NUMBER: {
                // the "created by text"
                let b1 = 0;
                let b2 = 0;
                while (b1 != 10 && b2 != 10) {
                ...
                index = 0;
                for (let v = 0; v < vertCount; v++) {
                    positions[index++] = reader.getFloat32(offset + 0, littleEndian);
                    positions[index++] = reader.getFloat32(offset + 4, littleEndian);
                    positions[index++] = reader.getFloat32(offset + 8, littleEndian);
                    offset += 12;
                }
                for (let f = 0; f < faceCount; f++) {
            ...
            case three_1.FreeSurferLoader.NEW_QUAD_FILE_MAGIC_NUMBER: {
...
three_1.FreeSurferLoader.QUAD_FILE_MAGIC_NUMBER = -1 & 0x00ffffff;
three_1.FreeSurferLoader.TRIANGLE_FILE_MAGIC_NUMBER = -2 & 0x00ffffff;
three_1.FreeSurferLoader.NEW_QUAD_FILE_MAGIC_NUMBER = -3 & 0x00ffffff;
